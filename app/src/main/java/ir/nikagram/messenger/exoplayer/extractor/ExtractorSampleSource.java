 ...
 */
package ir.nikagram.messenger.exoplayer.extractor;

import android.net.Uri;
import android.os.Handler;
import android.os.SystemClock;
import android.util.SparseArray;
import ir.nikagram.messenger.exoplayer.C;
import ir.nikagram.messenger.exoplayer.MediaFormat;
import ir.nikagram.messenger.exoplayer.MediaFormatHolder;
import ir.nikagram.messenger.exoplayer.ParserException;
import ir.nikagram.messenger.exoplayer.SampleHolder;
import ir.nikagram.messenger.exoplayer.SampleSource;
import ir.nikagram.messenger.exoplayer.TrackRenderer;
import ir.nikagram.messenger.exoplayer.drm.DrmInitData;
import ir.nikagram.messenger.exoplayer.upstream.Allocator;
import ir.nikagram.messenger.exoplayer.upstream.DataSource;
import ir.nikagram.messenger.exoplayer.upstream.DataSpec;
import ir.nikagram.messenger.exoplayer.upstream.Loader;
import ir.nikagram.messenger.exoplayer.util.Assertions;
import ir.nikagram.messenger.exoplayer.util.Util;
import java.io.EOFException;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import ir.nikagram.messenger.exoplayer.extractor.flv.FlvExtractor;
import ir.nikagram.messenger.exoplayer.extractor.mp3.Mp3Extractor;
import ir.nikagram.messenger.exoplayer.extractor.mp4.FragmentedMp4Extractor;
import ir.nikagram.messenger.exoplayer.extractor.mp4.Mp4Extractor;
import ir.nikagram.messenger.exoplayer.extractor.ogg.OggExtractor;
import ir.nikagram.messenger.exoplayer.extractor.ts.AdtsExtractor;
import ir.nikagram.messenger.exoplayer.extractor.ts.PsExtractor;
import ir.nikagram.messenger.exoplayer.extractor.ts.TsExtractor;
import ir.nikagram.messenger.exoplayer.extractor.wav.WavExtractor;
import ir.nikagram.messenger.exoplayer.extractor.webm.WebmExtractor;
 ...
 */
public final class ExtractorSampleSource implements SampleSource, SampleSource.SampleSourceReader,
  ...
  static {
    ...
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.webm.WebmExtractor")
    ...
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.mp4.FragmentedMp4Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.mp4.Mp4Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.mp3.Mp3Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.ts.AdtsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.ts.TsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.flv.FlvExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.ogg.OggExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.ts.PsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.extractor.wav.WavExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.nikagram.messenger.exoplayer.ext.flac.FlacExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
