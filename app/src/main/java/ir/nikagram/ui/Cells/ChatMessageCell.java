...
import ir.nikagram.ui.Components.LinkPath;
import ir.nikagram.ui.Components.RadialProgress;
import ir.nikagram.ui.Components.SeekBar;
import ir.nikagram.ui.Components.SeekBarWaveform;
import ir.nikagram.ui.Components.StaticLayoutEx;
import ir.nikagram.ui.ActionBar.Theme;
import ir.nikagram.ui.Components.TypefaceSpan;
...
import static ir.nikagram.messenger.AndroidUtilities.density;
import static ir.nikagram.messenger.AndroidUtilities.displaySize;
import static ir.nikagram.messenger.AndroidUtilities.dp;
import static ir.nikagram.messenger.AndroidUtilities.formatFileSize;
import static ir.nikagram.messenger.AndroidUtilities.getMinTabletSide;
import static ir.nikagram.messenger.AndroidUtilities.getPhotoSize;
import static ir.nikagram.messenger.AndroidUtilities.getTypeface;
import static ir.nikagram.messenger.AndroidUtilities.isTablet;
import static ir.nikagram.messenger.AndroidUtilities.replaceTags;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
    ...
    private int createDocumentLayout(int maxWidth, MessageObject messageObject) {
        ...
        if (MessageObject.isVoiceDocument(documentAttach)) {
            ...
            infoWidth = Math.min(maxWidth - dp(30), (int) Math.ceil(infoPaint.measureText(str)));
            CharSequence str2 = TextUtils.ellipsize(str, infoPaint, infoWidth, TextUtils.TruncateAt.END);
            try {
                if (infoWidth < 0) {
                    infoWidth = dp(10);
    ...
    public void setMessageObject(MessageObject messageObject) {
        ...
        if (messageChanged || dataChanged || isPhotoDataChanged(messageObject)) {
            ...
            if (messageObject.type == 0) {
                ...
                backgroundWidth = messageObject.textWidth + (hasGamePreview ? dp(10) : 0);
                ...
                if (docTitleLayout.getLineCount() > 0) {
                    int timeLeft = backgroundWidth - dp(40 + 18 + 44 + 8) - (int) Math.ceil(docTitleLayout.getLineWidth(0));
                ...
                if (messageObject.type == 9) {
                    ...
                    if (drawPhotoImage) {
                        ...
                        photoWidth = dp(56);
                        photoHeight = dp(56);
                        maxWidth += dp(TextUtils.isEmpty(messageObject.caption) ? 51 : 21);
                    }
                    availableTimeWidth = maxWidth;
                    if (!drawPhotoImage) {
                        if (TextUtils.isEmpty(messageObject.caption) && infoLayout.getLineCount() > 0) {
                            measureTime(messageObject);
                            int timeLeft = backgroundWidth - dp(40 + 18 + 56 + 8) - (int) Math.ceil(infoLayout.getLineWidth(0));
                            if (timeLeft < timeWidth) {
                                photoHeight += dp(8);
                            }
                        }
